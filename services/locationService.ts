...
export async function fetchProvinces(params?: {
    ...
    for (const item of list) {
        if (item && typeof item === 'object') {
            ...
            if (id > 0 || name)
                result.push({ id: id || result.length + 1, name });
        }
    }
    return result;
}
export async function fetchDistricts(provinceId: number, params?: {
    name?: string;
}): Promise<LocationOption[]> {
    const { data } = await apiClient.get<LocationOption[] | unknown[]>(`/api/provinces/${provinceId}/districts`, { params: { name: params?.name ?? '' } });
    return normalizeLocationList(data, 'districtId', 'districtName');
}
export async function fetchCities(districtId: number, params?: {
    name?: string;
}): Promise<LocationOption[]> {
    const { data } = await apiClient.get<LocationOption[] | unknown[]>(`/api/districts/${districtId}/cities`, { params: { name: params?.name ?? '' } });
    return normalizeLocationList(data, 'cityId', 'cityName');
}
function normalizeLocationList(raw: unknown, idKey: string, nameKey: string): LocationOption[] {
    if (!Array.isArray(raw))
        return [];
    return raw.map((item) => {
        if (item && typeof item === 'object') {
...
export async function geocodeCity(cityName: string, districtName?: string, provinceName?: string): Promise<{
    latitude: number;
    longitude: number;
} | null> {
