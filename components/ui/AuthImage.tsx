...
export function AuthImage({ type, fileName, token, style, resizeMode = 'cover', placeholder = null, }: AuthImageProps) {
    ...
    useEffect(() => {
        ...
        fetch(uri, {
            ...
            if (base64 && !cancelled) {
                if (__DEV__)
                    console.log('[Image Show API] Success', type, fileName);
                setDataUri(`data:${mime};base64,${base64}`);
            }
        })
            .catch((err) => {
            if (__DEV__)
                console.warn('[Image Show API] Error', type, fileName, err);
            if (!cancelled)
                setError(true);
        });
        return () => {
    ...
    return (<Image source={{ uri: dataUri }} style={style} contentFit={contentFit}/>);
}
