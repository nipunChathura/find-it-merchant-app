...
import { fetchMerchantDashboard, fetchRecentNotifications, markNotificationRead as markNotificationReadApi, } from '@/services/dashboardService';
import type { DashboardSummary, Notification, Outlet, PaymentItem, SubMerchantItem, UserRole, } from '@/types';
...
function useDashboardDataState(): DashboardDataValue {
    ...
    const markNotificationRead = useCallback(async (notification: Notification) => {
        ...
        if (!listId || !apiId)
            return;
        if (/^notif-\d+$/.test(listId))
            return;
        const current = notificationsRef.current.find((n) => n.id === listId);
        if (!current || current.read)
            return;
        if (markingReadIdsRef.current.has(listId))
            return;
