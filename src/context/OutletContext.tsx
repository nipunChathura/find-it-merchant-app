...
export function OutletProvider({ children }: {
    ...
    const addOutlet = useCallback(async (o: Omit<OutletRecord, 'id'>): Promise<OutletRecord> => {
    ...
    const assignOutletToSubMerchant = useCallback(async (outletId: string, subMerchantUserId: string | null) => {
        setData((prev) => {
            const outlets = prev.outlets.map((o) => o.id === outletId ? { ...o, assignedToSubMerchantId: subMerchantUserId } : o);
            const next = { ...prev, outlets };
            AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(next)).catch(() => { });
            return next;
        });
    }, []);
    const getOutletsForRole = useCallback((role: UserRole, currentUserId: string | null): OutletRecord[] => {
        if (role === 'SUBMERCHANT' && currentUserId) {
            return data.outlets.filter((o) => o.assignedToSubMerchantId === currentUserId);
        }
        return data.outlets;
    }, [data.outlets]);
    const getOutletById = useCallback((id: string) => data.outlets.find((o) => o.id === id), [data.outlets]);
    const getItemsByOutletId = useCallback((outletId: string) => data.items.filter((i) => i.outletId === outletId), [data.items]);
    ...
    const getSchedulesByOutletId = useCallback((outletId: string) => data.schedules.filter((s) => s.outletId === outletId), [data.schedules]);
    ...
    const getPaymentsByOutletId = useCallback((outletId: string) => data.payments.filter((p) => p.outletId === outletId).sort((a, b) => b.createdAt.localeCompare(a.createdAt)), [data.payments]);
    const addPayment = useCallback(async (p: Omit<PaymentRecord, 'id' | 'createdAt'>): Promise<PaymentRecord> => {
    ...
    const paidOutletIds = new Set(data.payments.filter((p) => p.month === thisMonth).map((p) => p.outletId));
