...
function InfoRow({ icon, label, value }: {
    icon: 'person' | 'email' | 'phone' | 'location-on' | 'badge' | 'store';
    label: string;
    value?: string | null;
}) {
    if (value == null || value === '')
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.profileHeader}>
          <View style={styles.avatarWrap}>
            {user?.profileImage && token ? (<AuthImage type="profile" fileName={user.profileImage} token={token} style={styles.avatarImage} resizeMode="cover" placeholder={<View style={styles.avatar}>
                    <Text style={styles.avatarText}>
                      {displayName?.charAt(0).toUpperCase() ?? 'M'}
                    </Text>
                  </View>}/>) : user?.profileImageUri ? (<Image source={{ uri: user.profileImageUri }} style={styles.avatarImage} contentFit="cover"/>) : (<View style={styles.avatar}>
                <Text style={styles.avatarText}>
                  {displayName?.charAt(0).toUpperCase() ?? 'M'}
                </Text>
              </View>)}
        ...
        <Pressable style={({ pressed }) => [styles.menuButton, pressed && styles.buttonPressed]} onPress={() => router.push('/(tabs)/profile/change-image')}>
          <MaterialIcons name="photo-camera" size={22} color={colors.primary}/>
          <Text style={styles.menuText}>Change profile image</Text>
        </Pressable>
        <Pressable style={({ pressed }) => [styles.menuButton, pressed && styles.buttonPressed]} onPress={() => router.push('/(tabs)/profile/change-password')}>
          <MaterialIcons name="lock" size={22} color={colors.primary}/>
          <Text style={styles.menuText}>Change password</Text>
        </Pressable>
        <Pressable style={({ pressed }) => [styles.menuButton, pressed && styles.buttonPressed]} onPress={() => router.push('/(tabs)/profile/edit')}>
          <MaterialIcons name="edit" size={22} color={colors.primary}/>
          <Text style={styles.menuText}>Edit profile</Text>
        </Pressable>

        <Pressable style={({ pressed }) => [styles.logoutButton, pressed && styles.buttonPressed]} onPress={async () => {
            await signOut();
