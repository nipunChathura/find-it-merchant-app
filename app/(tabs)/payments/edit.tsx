...
import { deletePayment, updatePayment, uploadImage, type OutletPaymentDetail, type UpdatePaymentPayload, } from '@/services/paymentService';
...
export default function EditPaymentScreen() {
    ...
    useEffect(() => {
        if (!payment || outlets.length === 0)
            return;
        const found = outlets.find((o) => o.id === String(payment.outletId) || parseInt(o.id, 10) === payment.outletId);
        if (found) {
            setSelectedOutlet((prev) => prev ? prev : { id: payment.outletId, name: found.name });
        }
    }, [payment, outlets]);
    ...
    if (!payment) {
        return (<View style={[styles.screen, { paddingTop: insets.top + spacing.md }]}>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={styles.card}>
            ...
            <View style={styles.cardInner}>
              ...
              <SearchableDropdown options={outletOptions} selected={selectedOutlet} onSelect={setSelectedOutlet} placeholder={outletsLoading ? 'Loading…' : 'Select outlet'} searchPlaceholder="Search outlet" disabled={loading} loading={outletsLoading}/>
            </View>
          </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.cardInner}>
              ...
              <AppInput placeholder="e.g. SUBSCRIPTION" value={paymentType} onChangeText={(t) => { setPaymentType(t); setError(''); }} editable={!loading} style={styles.input}/>
              <Text style={styles.label}>Amount (LKR) *</Text>
              <AppInput placeholder="0.00" value={amount} onChangeText={(t) => { setAmount(t); setError(''); }} keyboardType="decimal-pad" editable={!loading} style={styles.input}/>
              <Text style={styles.label}>Payment date</Text>
              <AppInput placeholder="YYYY-MM-DD" value={paymentDate} onChangeText={setPaymentDate} editable={!loading} style={styles.input}/>
              <Text style={styles.label}>Paid month</Text>
              <AppInput placeholder="YYYY-MM" value={paidMonth} onChangeText={setPaidMonth} editable={!loading} style={styles.input}/>
              <Text style={styles.label}>Status</Text>
              <AppInput placeholder="e.g. PENDING, PAID" value={status} onChangeText={setStatus} editable={!loading} style={styles.input}/>
            </View>
          </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.cardInner}>
              ...
              {receiptUri ? (<View style={styles.imageWrap}>
                  <Image source={{ uri: receiptUri }} style={styles.image} resizeMode="cover"/>
                  {receiptImageName ? (<Text style={styles.imageName} numberOfLines={1}>{receiptImageName}</Text>) : null}
                  <PrimaryButton title={uploadingImage ? 'Uploading…' : 'Change image'} onPress={pickAndUploadImage} disabled={uploadingImage || loading}/>
                </View>) : (<>
                  {receiptImageName && token ? (<View style={styles.imageWrap}>
                      <AuthImage type="receipt" fileName={receiptImageName} token={token} style={styles.image} resizeMode="cover" placeholder={<View style={styles.imagePlaceholder}>
                            <MaterialIcons name="receipt" size={48} color={colors.textSecondary}/>
                            <Text style={styles.imageName}>Current: {receiptImageName}</Text>
                          </View>}/>
                    </View>) : receiptImageName ? (<Text style={styles.imageName}>Current: {receiptImageName}</Text>) : null}
                  <PrimaryButton title={uploadingImage ? 'Uploading…' : receiptImageName ? 'Change receipt' : 'Upload receipt'} onPress={pickAndUploadImage} disabled={uploadingImage || loading}/>
                </>)}
            </View>
          </View>

          {error ? (<View style={styles.errorWrap}>
              <MaterialIcons name="error-outline" size={18} color={colors.error}/>
              <Text style={styles.errorText}>{error}</Text>
            </View>) : null}

          <PrimaryButton title={loading ? 'Saving…' : 'Save changes'} onPress={handleSave} loading={loading} disabled={loading}/>
          {role === 'MERCHANT' ? (<Pressable onPress={handleDelete} disabled={loading} style={[styles.deleteBtn, loading && styles.deleteBtnDisabled]}>
