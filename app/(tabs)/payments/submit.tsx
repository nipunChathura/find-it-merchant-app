...
export default function SubmitPaymentScreen() {
    ...
    useEffect(() => {
        const idParam = params.outletId;
        if (!idParam || outlets.length === 0)
            return;
        const outletIdNum = parseInt(idParam, 10);
        if (isNaN(outletIdNum))
            return;
        const found = outlets.find((o) => o.id === idParam || parseInt(o.id, 10) === outletIdNum);
        if (found) {
            setSelectedOutlet((prev) => (prev ? prev : { id: outletIdNum, name: found.name }));
        }
    }, [params.outletId, outlets]);
    ...
    return (<View style={styles.screen}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboard}>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.cardInner}>
              ...
              {receiptUri ? (<View style={styles.imageWrap}>
                ...
                </View>) : (<PrimaryButton title={uploadingImage ? 'Uploading…' : 'Upload receipt'} onPress={pickAndUploadImage} disabled={uploadingImage || loading}/>)}
            </View>
          </View>
          ...
          <PrimaryButton title={loading ? 'Submitting…' : 'Submit Payment'} onPress={handleSubmit} loading={loading} disabled={loading}/>
        </ScrollView>
