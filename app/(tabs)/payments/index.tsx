    ...
    return (<ScreenContainer>
      <View style={styles.section}>
        <View style={styles.header}>
          ...
          <Pressable onPress={() => router.push('/(tabs)/payments/submit')} style={({ pressed }) => [styles.addButton, pressed && styles.addButtonPressed]}>
            <Text style={styles.addButtonText}>Submit Payment</Text>
          </Pressable>
        </View>
        {sortedPayments.length === 0 ? (<EmptyState icon="payment" title="No payments yet" message="Submit a payment for an outlet to record it here."/>) : (sortedPayments.map((p, idx) => (<View key={p.paymentId ? `payment-${p.paymentId}` : `payment-${idx}`} style={styles.card}>
              ...
              {p.paymentStatus ? (<Text style={styles.cardStatus}>{p.paymentStatus}</Text>) : null}
              {p.paymentStatus === 'PENDING' ? (<View style={styles.cardActions}>
                  ...
                  {role === 'MERCHANT' ? (<Pressable onPress={() => {
