...
import { approveSubMerchant, deleteSubMerchant, rejectSubMerchant, } from '@/services/subMerchantService';
...
export default function SubMerchantDetailsScreen() {
    ...
    const item: SubMerchantItem | undefined = Number.isNaN(subMerchantId) ? undefined : subMerchants.find((sm) => sm.subMerchantId === subMerchantId);
    if (role !== 'MERCHANT') {
    ...
    const handleDelete = () => {
        Alert.alert('Delete Sub Merchant', `Remove "${item.merchantName || 'this sub merchant'}"? This cannot be undone.`, [
    ...
    const handleReject = async () => {
        Alert.alert('Reject Sub Merchant', `Reject "${item.merchantName || 'this sub merchant'}"?`, [
            { text: 'Cancel', style: 'cancel' },
            {
                text: 'Reject',
                style: 'destructive',
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.heroCard}>
          <View style={styles.avatarLarge}>
            {item.profileImage && token ? (<AuthImage type="profile" fileName={item.profileImage} token={token} style={styles.avatarLargeImage} resizeMode="cover" placeholder={<View style={styles.avatarLargePlaceholder}>
                    <Text style={styles.avatarLargeText}>{initial}</Text>
                  </View>}/>) : (<Text style={styles.avatarLargeText}>{initial}</Text>)}
          </View>
        ...
        {isMerchant ? (<View style={styles.card}>
            <View style={styles.cardAccent}/>
            <View style={styles.cardInner}>
              <View style={styles.sectionLabel}>
              ...
              <Pressable onPress={() => router.push({ pathname: '/(tabs)/sub-merchants/edit', params: { id: String(item.subMerchantId) } })} style={styles.actionRow} disabled={actionLoading}>
                <MaterialIcons name="edit" size={22} color={colors.primary}/>
                <Text style={styles.actionText}>Change details</Text>
                <MaterialIcons name="chevron-right" size={20} color={colors.textSecondary}/>
              </Pressable>
              {isPending ? (<View style={styles.pendingActionsRow}>
                  <Pressable style={[styles.approveBtn, actionLoading && styles.actionBtnDisabled]} onPress={handleApprove} disabled={actionLoading}>
                    <MaterialIcons name="check-circle" size={18} color={colors.white}/>
                    <Text style={styles.approveBtnText}>{actionLoading ? 'Approving…' : 'Approve'}</Text>
                  </Pressable>
                  <Pressable style={[styles.rejectBtn, actionLoading && styles.actionBtnDisabled]} onPress={handleReject} disabled={actionLoading}>
                    <MaterialIcons name="cancel" size={18} color={colors.white}/>
                    <Text style={styles.rejectBtnText}>Reject</Text>
                  </Pressable>
                </View>) : null}
              <View style={styles.deleteWrap}>
                <Pressable onPress={handleDelete} style={styles.deleteButton} disabled={actionLoading}>
