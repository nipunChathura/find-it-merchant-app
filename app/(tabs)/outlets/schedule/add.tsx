...
import { createSchedule, isValidTimeHHmm, SCHEDULE_DAYS, SCHEDULE_TYPES, type CreateSchedulePayload, type CreateScheduleResponse, type ScheduleType, } from '@/services/scheduleService';
...
export default function AddScheduleScreen() {
    ...
    const buildPayload = useCallback((dayOfWeek: string | null): CreateSchedulePayload => ({
        scheduleType,
        dayOfWeek,
      ...
      <ScreenContainer>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.typeRow}>
            {SCHEDULE_TYPES.map((t) => (<Pressable key={t} style={[styles.typeChip, scheduleType === t && styles.typeChipActive]} onPress={() => setScheduleType(t)} disabled={loading}>
          ...
          {scheduleType === 'NORMAL' && (<>
              ...
              <View style={styles.dayRowWrap}>
                {SCHEDULE_DAYS.map((d) => (<Pressable key={d} style={[styles.dayChip, selectedDays.has(d) && styles.dayChipActive]} onPress={() => toggleDay(d)} disabled={loading}>
                    <Text style={[styles.dayChipText, selectedDays.has(d) && styles.dayChipTextActive]}>{d.slice(0, 3)}</Text>
                  </Pressable>))}
              </View>
            </>)}

          {(scheduleType === 'EMERGENCY' || scheduleType === 'DAILY') && (<DatePickerField label="Date" value={specialDate} onChange={setSpecialDate} disabled={loading}/>)}

          {scheduleType === 'TEMPORARY' && (<>
              <DatePickerField label="Start date" value={startDate} onChange={setStartDate} disabled={loading}/>
              <DatePickerField label="End date" value={endDate} onChange={setEndDate} disabled={loading} minimumDate={parseYYYYMMDDToDate(startDate)}/>
            </>)}
          ...
          {(scheduleType === 'EMERGENCY' || scheduleType === 'DAILY' || scheduleType === 'TEMPORARY') && (<>
              ...
              <AppInput placeholder="e.g. Special event" value={reason} onChangeText={setReason} style={styles.input} editable={!loading} multiline/>
            </>)}
