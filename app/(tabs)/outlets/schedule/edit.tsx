...
import { fetchOutletScheduleDetails, findScheduleSlotById, isValidTimeHHmm, SCHEDULE_DAYS, SCHEDULE_TYPES, updateScheduleSlot, type CreateSchedulePayload, type NormalScheduleSlot, type ScheduleType, type SpecialScheduleSlot, } from '@/services/scheduleService';
import { colors } from '@/theme/colors';
import { layout, spacing } from '@/theme/spacing';
import { fontSizes, fontWeights } from '@/theme/typography';
import { normalizeApiTimeToHHmm, parseYYYYMMDDToDate, toYYYYMMDD } from '@/utils/scheduleDateTime';
export default function EditScheduleScreen() {
    const { id, outletId } = useLocalSearchParams<{
        id: string;
        outletId: string;
    }>();
    ...
    const handleSave = async () => {
        if (!outletId || Number.isNaN(scheduleId))
            return;
        setError('');
        if (!validate())
            return;
      ...
      <ScreenContainer>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.typeRow}>
            {SCHEDULE_TYPES.map((t) => (<Pressable key={t} style={[styles.typeChip, scheduleType === t && styles.typeChipActive]} onPress={() => setScheduleType(t)} disabled={saving}>
          ...
          {scheduleType === 'NORMAL' && (<>
              ...
              <View style={styles.dayRowWrap}>
                {SCHEDULE_DAYS.map((d) => (<Pressable key={d} style={[styles.dayChip, dayOfWeek === d && styles.dayChipActive]} onPress={() => setDayOfWeek(d)} disabled={saving}>
                    <Text style={[styles.dayChipText, dayOfWeek === d && styles.dayChipTextActive]}>{d.slice(0, 3)}</Text>
                  </Pressable>))}
              </View>
            </>)}

          {(scheduleType === 'EMERGENCY' || scheduleType === 'DAILY') && (<DatePickerField label="Date" value={specialDate} onChange={setSpecialDate} disabled={saving}/>)}

          {scheduleType === 'TEMPORARY' && (<>
              <DatePickerField label="Start date" value={startDate} onChange={setStartDate} disabled={saving}/>
              <DatePickerField label="End date" value={endDate} onChange={setEndDate} disabled={saving} minimumDate={parseYYYYMMDDToDate(startDate)}/>
            </>)}
          ...
          {(scheduleType === 'EMERGENCY' || scheduleType === 'DAILY' || scheduleType === 'TEMPORARY') && (<>
              ...
              <AppInput placeholder="e.g. Special event" value={reason} onChangeText={setReason} style={styles.input} editable={!saving} multiline/>
            </>)}
