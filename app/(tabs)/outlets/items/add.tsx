      ...
      <View style={styles.root}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} enabled={Platform.OS === 'ios'} style={styles.keyboard}>
          <ScrollView style={styles.scrollView} contentContainerStyle={[
            ...
            <AppInput placeholder="Item Name *" value={name} onChangeText={(t) => { setName(t); setError(''); }} editable={!loading} style={styles.input}/>
            ...
            <SearchableDropdown options={categoryOptions} selected={selectedCategory} onSelect={setSelectedCategory} placeholder="Select category" searchPlaceholder="Search category" loading={categoryOptionsLoading} disabled={loading} onOpen={() => categoryOptions.length === 0 && loadCategories()}/>
            ...
            {imageUri ? (<View style={styles.imageWrap}>
              ...
              </View>) : (<Pressable onPress={pickImage} disabled={loading} style={({ pressed }) => [styles.imagePlaceholder, pressed && styles.imagePlaceholderPressed]}>
                <MaterialIcons name="add-a-photo" size={40} color={colors.textSecondary}/>
                <Text style={styles.imagePlaceholderText}>Select image</Text>
              </Pressable>)}

            <AppInput placeholder="Description" value={description} onChangeText={setDescription} multiline editable={!loading} style={[styles.input, styles.inputMultiline]}/>
            ...
            <PrimaryButton title={loading ? (uploadingImage ? 'Uploading…' : 'Saving…') : 'Save Item'} onPress={handleSave} loading={loading} disabled={loading}/>
          </ScrollView>
