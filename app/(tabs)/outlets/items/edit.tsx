...
export default function EditItemScreen() {
    ...
    const [price, setPrice] = useState(itemFromApi != null && itemFromApi.price != null && !Number.isNaN(Number(itemFromApi.price))
        ? String(itemFromApi.price)
        : '');
    const [categoryId, setCategoryId] = useState(itemFromApi?.categoryId != null ? String(itemFromApi.categoryId) : '');
    const [categoryOptions, setCategoryOptions] = useState<DropdownOption[]>([]);
    const [categoriesLoading, setCategoriesLoading] = useState(false);
    const [availability, setAvailability] = useState(itemFromApi?.availability ?? true);
    const [status, setStatus] = useState<'ACTIVE' | 'INACTIVE'>((itemFromApi?.status?.toUpperCase() === 'INACTIVE' ? 'INACTIVE' : 'ACTIVE') as 'ACTIVE' | 'INACTIVE');
    ...
    return (<SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.root}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} enabled={Platform.OS === 'ios'} style={styles.keyboard}>
          <ScrollView style={styles.scrollView} contentContainerStyle={[
            ...
            <AppInput placeholder="Enter item name" value={itemName} onChangeText={(t) => { setItemName(t); setError(''); }} editable={!loading} style={styles.input}/>
            <ThemedText style={styles.label}>Price (LKR)</ThemedText>
            <AppInput placeholder="Price (LKR, optional)" value={price} onChangeText={setPrice} keyboardType="decimal-pad" editable={!loading} style={styles.input}/>
            <ThemedText style={styles.label}>Category</ThemedText>
            <SearchableDropdown options={categoryDropdownOptions} selected={selectedCategory} onSelect={(opt) => setCategoryId(opt.id === 0 ? '' : String(opt.id))} placeholder="Select category" searchPlaceholder="Search category" disabled={loading} loading={categoriesLoading} onOpen={() => loadCategoryOptions()}/>

            <ThemedText style={styles.label}>Item image</ThemedText>
            {imageUri ? (<View style={styles.imageWrap}>
                <Image source={{ uri: imageUri }} style={styles.imagePreview} resizeMode="cover"/>
                <View style={styles.imageActions}>
                  <PrimaryButton title={uploadingImage ? 'Uploading…' : 'Change image'} onPress={pickImage} disabled={loading || uploadingImage}/>
                  <Pressable onPress={() => setImageUri(null)} disabled={loading} style={styles.removeImageBtn}>
                    <Text style={styles.removeImageText}>Remove</Text>
                  </Pressable>
                </View>
              </View>) : (<View style={styles.imageWrap}>
                {itemFromApi?.itemImage && token ? (<View style={styles.currentImageWrap}>
                    <AuthImage type="item" fileName={itemFromApi.itemImage} token={token} style={styles.imagePreview} resizeMode="cover" placeholder={<View style={[styles.imagePreview, styles.imagePlaceholder]}>
                          <MaterialIcons name="restaurant" size={40} color={colors.textSecondary}/>
                        </View>}/>
                    <Text style={styles.currentImageLabel}>Current image</Text>
                  </View>) : null}
                <PrimaryButton title="Select new image" onPress={pickImage} disabled={loading}/>
              </View>)}

            <ThemedText style={styles.label}>Description</ThemedText>
            <AppInput placeholder="Enter description" value={itemDescription} onChangeText={setItemDescription} multiline editable={!loading} style={[styles.input, styles.inputMultiline]}/>

            <ThemedText style={styles.label}>Availability</ThemedText>
            <View style={styles.row}>
              <TouchableOpacity style={[styles.toggleBtn, availability && styles.toggleBtnActive]} onPress={() => setAvailability(true)} disabled={loading}>
                <Text style={[styles.toggleText, availability && styles.toggleTextActive]}>Available</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.toggleBtn, !availability && styles.toggleBtnActive]} onPress={() => setAvailability(false)} disabled={loading}>
            ...
            <View style={styles.row}>
              <TouchableOpacity style={[styles.toggleBtn, status === 'ACTIVE' && styles.toggleBtnActive]} onPress={() => setStatus('ACTIVE')} disabled={loading}>
                <Text style={[styles.toggleText, status === 'ACTIVE' && styles.toggleTextActive]}>ACTIVE</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.toggleBtn, status === 'INACTIVE' && styles.toggleBtnActive]} onPress={() => setStatus('INACTIVE')} disabled={loading}>
            ...
            <PrimaryButton title={loading ? (uploadingImage ? 'Uploading…' : 'Saving…') : 'Save Changes'} onPress={handleSave} loading={loading} disabled={loading}/>
          </ScrollView>
