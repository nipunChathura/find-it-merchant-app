...
const outletTypeDropdownOptions: DropdownOption[] = OUTLET_TYPE_OPTIONS.map((o, i) => ({
  id: i,
  name: o.label,
}));
const businessCategoryDropdownOptions: DropdownOption[] = BUSINESS_CATEGORY_OPTIONS.map((o, i) => ({
  id: i,
  name: o.label,
}));

/** Form lives here so typing does not re-render the screen shell (header / safe area). */
export function AddOutletFormBody() {
  ...
  const outletForm = (
    <>
      <AddOutletFormSection title="Basic information">
        ...
        <AppInput
          ...
          style={styles.input}
        />
      </AddOutletFormSection>

      <AddOutletFormSection title="Address">
        <AppInput
      ...
      <AddOutletFormSection title="Business details">
        <AppInput
        ...
        <SearchableDropdown
          options={outletTypeDropdownOptions}
          selected={
            (() => {
        ...
        <SearchableDropdown
          options={businessCategoryDropdownOptions}
          selected={
            (() => {
      ...
      <AddOutletFormSection title="Location (coordinates)">
        {selectedCity ? (
          <>
      ...
      <AddOutletFormSection title="Bank details">
        <AppInput
      ...
      <AddOutletFormSection title="Remarks">
        <AppInput
  ...
  const FormScroll = Platform.OS === 'android' ? GHScrollView : ScrollView;

  return (
    <View style={styles.formRoot}>
      {Platform.OS === 'ios' ? (
        <KeyboardAvoidingView behavior="padding" style={styles.keyboard} keyboardVerticalOffset={0}>
          <FormScroll {...scrollCommon} automaticallyAdjustKeyboardInsets contentInsetAdjustmentBehavior="never">
            {outletForm}
          </FormScroll>
        </KeyboardAvoidingView>
      ) : (
        <FormScroll {...scrollCommon} overScrollMode="never">
          {outletForm}
        </FormScroll>
      )}
...
function LocationMapPicker({
  ...
  const mapRef = useRef<{ animateToRegion: (r: unknown, d?: number) => void } | null>(null);
  const lastAnimatedGeocodeKey = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (geocodeLoading || !mapCenter) return;
    const key = `${mapCenter.latitude.toFixed(5)},${mapCenter.longitude.toFixed(5)}`;
    if (lastAnimatedGeocodeKey.current === key) return;
    lastAnimatedGeocodeKey.current = key;
  ...
  }, [mapCenter?.latitude, mapCenter?.longitude, geocodeLoading]);
...
function AddOutletFormSection({
  ...
  return (
    <View style={styles.section} collapsable={false}>
      ...
      <View style={styles.sectionCard} collapsable={false}>
        {children}
      </View>
    </View>
  );
