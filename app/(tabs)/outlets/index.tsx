    ...
    return (<ScreenContainer>
      <View style={styles.section}>
        <View style={styles.header}>
          ...
          {isMerchant ? (<Pressable onPress={() => router.push('/(tabs)/outlets/add')} style={({ pressed }) => [styles.addButton, pressed && styles.addButtonPressed]}>
              <Text style={styles.addButtonText}>Add Outlet</Text>
            </Pressable>) : null}
        </View>
        {loading && outlets.length === 0 ? (<View style={styles.loadingWrap}>
            <LoadingSpinner />
          </View>) : outlets.length === 0 ? (<EmptyState icon="store" title="No outlet data" message={role === 'SUBMERCHANT'
                ? 'No outlets assigned to you yet. Data will appear here once assigned.'
                : 'No outlet data available. Add your first outlet to get started.'}/>) : (outlets.map((outlet, idx) => (<OutletCard key={`outlet-${idx}-${outlet?.id ?? 'noid'}`} outlet={outlet} onPress={() => router.push(`/(tabs)/outlets/${outlet.id}`)}/>)))}
      </View>
    </ScreenContainer>);
}
