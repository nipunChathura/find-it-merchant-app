...
import { fetchCities, fetchDistricts, fetchProvinces, geocodeCity, type LocationOption, } from '@/services/locationService';
...
export default function EditOutletScreen() {
        ...
        try {
            ...
            if (details?.outlet) {
                ...
                if (o.provinceId != null)
                    setInitialProvinceId(o.provinceId);
                if (o.districtId != null)
                    setInitialDistrictId(o.districtId);
                if (o.cityId != null)
                    setInitialCityId(o.cityId);
                if (o.latitude != null)
    ...
    useEffect(() => {
        if (subMerchants.length === 0 && assignedSubMerchantId == null)
            return;
    ...
    const onSubMerchantSelect = useCallback((opt: DropdownOption) => {
        ...
        Alert.alert('Change assigned sub merchant', `Assign this outlet to "${newName}"? The current assignment will be replaced.`, [
    ...
    useEffect(() => {
        if (provinces.length === 0 || initialProvinceId == null || selectedProvince != null)
            return;
    ...
    useEffect(() => {
        if (districts.length === 0 || initialDistrictId == null || selectedDistrict != null)
            return;
        const match = districts.find((d) => d.id === initialDistrictId);
        if (match)
            setSelectedDistrict({ id: match.id, name: match.name });
    ...
    useEffect(() => {
        if (cities.length === 0 || initialCityId == null || selectedCity != null)
            return;
        const match = cities.find((c) => c.id === initialCityId);
        if (match)
            setSelectedCity({ id: match.id, name: match.name });
    ...
    useEffect(() => {
        ...
        geocodeCity(selectedCity.name, selectedDistrict?.name ?? undefined, selectedProvince?.name ?? undefined)
            .then((coords) => {
            if (cancelled || !coords)
                return;
            setMapCenter(coords);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboard}>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <Section title="Basic information">
            <AppInput placeholder="Outlet name *" value={name} onChangeText={(t) => {
            setName(t);
            setError('');
        }} editable={!loading} style={styles.input}/>
            <AppInput placeholder="Contact number" value={contactNumber} onChangeText={setContactNumber} keyboardType="phone-pad" editable={!loading} style={styles.input}/>
            <AppInput placeholder="Email address" value={emailAddress} onChangeText={setEmailAddress} keyboardType="email-address" autoCapitalize="none" editable={!loading} style={styles.input}/>
            <AppInput placeholder="Description (optional)" value={description} onChangeText={setDescription} multiline editable={!loading} style={[styles.input, styles.inputMultiline]}/>
          </Section>

          <Section title="Address">
            <AppInput placeholder="Address line 1" value={location} onChangeText={setLocation} editable={!loading} style={styles.input}/>
            <AppInput placeholder="Postal code" value={postalCode} onChangeText={setPostalCode} editable={!loading} style={styles.input}/>
            <Text style={styles.dropdownLabel}>Province</Text>
            <SearchableDropdown options={provinces} selected={selectedProvince} onSelect={onProvinceSelect} placeholder="Select province" searchPlaceholder="Search province" loading={provincesLoading} disabled={loading} onOpen={() => provinces.length === 0 && loadProvinces()}/>
            <Text style={[styles.dropdownLabel, { marginTop: spacing.md }]}>District</Text>
            <SearchableDropdown options={districts} selected={selectedDistrict} onSelect={onDistrictSelect} placeholder="Select district" searchPlaceholder="Search district" loading={districtsLoading} disabled={loading || !selectedProvince} onOpen={() => selectedProvince && districts.length === 0 && loadDistricts()}/>
            <Text style={[styles.dropdownLabel, { marginTop: spacing.md }]}>City</Text>
            <SearchableDropdown options={cities} selected={selectedCity} onSelect={setSelectedCity} placeholder="Select city" searchPlaceholder="Search city" loading={citiesLoading} disabled={loading || !selectedDistrict} onOpen={() => selectedDistrict && cities.length === 0 && loadCities()}/>
          </Section>
          ...
          <Section title="Location (coordinates)">
            {selectedCity ? (<>
                ...
                <LocationMapPicker mapCenter={mapCenter} latitude={latitude} longitude={longitude} geocodeLoading={geocodeLoading} onMarkerDragEnd={(lat, lng) => {
            ...
            <View style={styles.row}>
              <AppInput placeholder="Latitude" value={latitude} onChangeText={setLatitude} keyboardType="decimal-pad" editable={!loading} style={[styles.input, styles.inputHalf]}/>
              <AppInput placeholder="Longitude" value={longitude} onChangeText={setLongitude} keyboardType="decimal-pad" editable={!loading} style={[styles.input, styles.inputHalf]}/>
            </View>
          </Section>

          <Section title="Business details">
            <AppInput placeholder="Business registration number" value={businessRegistrationNumber} onChangeText={setBusinessRegistrationNumber} editable={!loading} style={styles.input}/>
            <AppInput placeholder="Tax identification number" value={taxIdentificationNumber} onChangeText={setTaxIdentificationNumber} editable={!loading} style={styles.input}/>
            <Text style={styles.dropdownLabel}>Outlet type</Text>
            <SearchableDropdown options={OUTLET_TYPE_OPTIONS.map((o, i) => ({ id: i, name: o.label }))} selected={(() => {
            const idx = OUTLET_TYPE_OPTIONS.findIndex((o) => o.value === outletType);
            return idx >= 0 ? { id: idx, name: OUTLET_TYPE_OPTIONS[idx].label } : null;
        })()} onSelect={(opt) => setOutletType(OUTLET_TYPE_OPTIONS[opt.id].value)} placeholder="Select outlet type" searchPlaceholder="Search outlet type" disabled={loading}/>
            <Text style={[styles.dropdownLabel, { marginTop: spacing.md }]}>Business category</Text>
            <SearchableDropdown options={BUSINESS_CATEGORY_OPTIONS.map((o, i) => ({ id: i, name: o.label }))} selected={(() => {
        ...
        })()} onSelect={(opt) => setBusinessCategory(BUSINESS_CATEGORY_OPTIONS[opt.id].value)} placeholder="Select business category" searchPlaceholder="Search business category" disabled={loading}/>
          </Section>

          {role === 'MERCHANT' ? (<Section title="Assign sub merchant">
              <Text style={styles.dropdownLabel}>Assigned sub merchant</Text>
              <SearchableDropdown options={subMerchantOptions} selected={selectedSubMerchant} onSelect={onSubMerchantSelect} placeholder="Select sub merchant" searchPlaceholder="Search sub merchant" loading={subMerchantsLoading} disabled={loading || subMerchantUpdateLoading} onOpen={() => subMerchants.length === 0 && loadSubMerchants()}/>
              {subMerchantUpdateLoading ? (<Text style={styles.mapHint}>Updating assignment…</Text>) : null}
            </Section>) : null}

          {error ? <Text style={styles.error}>{error}</Text> : null}

          <View style={styles.submitWrap}>
            <PrimaryButton title={loading ? 'Saving…' : 'Save Changes'} onPress={handleSave} loading={loading} disabled={loading}/>
          </View>
        </ScrollView>
...
function LocationMapPicker({ mapCenter, latitude, longitude, geocodeLoading, onMarkerDragEnd, disabled, }: {
    mapCenter: {
        latitude: number;
        longitude: number;
    } | null;
    ...
    try {
        ...
        {geocodeLoading ? (<Text style={styles.mapLoadingText}>Loading map location…</Text>) : null}
        <MapView key={`${center.latitude.toFixed(4)}-${center.longitude.toFixed(4)}`} style={styles.map} initialRegion={{
                latitude: center.latitude,
                longitude: center.longitude,
                latitudeDelta: MAP_DELTA,
                longitudeDelta: MAP_DELTA,
            }} scrollEnabled={!disabled} zoomEnabled={!disabled} onPress={(e) => {
                if (disabled)
                    return;
          ...
          <Marker coordinate={{
            ...
            }} draggable={!disabled} onDragEnd={(e) => {
                const { latitude: lat, longitude: lng } = e.nativeEvent.coordinate;
                onMarkerDragEnd(lat, lng);
