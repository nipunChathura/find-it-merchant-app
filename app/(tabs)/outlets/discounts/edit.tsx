...
import { type CreateOrUpdateDiscountPayload, deleteDiscount, type OutletDiscountDetail, updateDiscount, } from '@/services/discountService';
...
export default function EditDiscountScreen() {
    ...
    const [discountType, setDiscountType] = useState<'PERCENTAGE' | 'FIXED_AMOUNT'>(discount?.discountType === 'FIXED_AMOUNT' ? 'FIXED_AMOUNT' : 'PERCENTAGE');
    const [discountValue, setDiscountValue] = useState(discount?.discountValue != null ? String(discount.discountValue) : '');
    ...
    const addSelectedItem = useCallback((item: ItemApiDto) => {
        setSelectedItems((prev) => {
            if (prev.some((s) => s.itemId === item.itemId))
                return prev;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const data = err && typeof err === 'object' && 'response' in err
                ...
                : undefined;
            setError(typeof data?.responseMessage === 'string' ? data.responseMessage : 'Failed to update discount');
        }
        finally {
            setLoading(false);
        }
    ...
    const handleDelete = () => {
        if (!discount)
            return;
        Alert.alert('Delete discount', `Are you sure you want to delete "${discount.discountName ?? 'this discount'}"?`, [
    ...
    if (!discount) {
        return (<View style={[styles.container, { paddingTop: insets.top + spacing.md }]}>
    ...
    return (<View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <AppInput placeholder="Discount name" value={discountName} onChangeText={(t) => { setDiscountName(t); setError(''); }} editable={!loading} style={styles.input}/>

        <Text style={styles.label}>Discount type</Text>
        <View style={styles.typeRow}>
          {DISCOUNT_TYPES.map((type) => (<Pressable key={type} onPress={() => { setDiscountType(type); setError(''); }} style={[styles.typeBtn, discountType === type && styles.typeBtnActive]}>
        ...
        <AppInput placeholder={discountType === 'PERCENTAGE' ? 'Value (e.g. 15)' : 'Value in LKR'} value={discountValue} onChangeText={(t) => { setDiscountValue(t); setError(''); }} keyboardType="decimal-pad" editable={!loading} style={styles.input}/>

        <AppInput placeholder="Start date (YYYY-MM-DD)" value={startDate} onChangeText={(t) => { setStartDate(t); setError(''); }} editable={!loading} style={styles.input}/>
        <AppInput placeholder="End date (YYYY-MM-DD)" value={endDate} onChangeText={(t) => { setEndDate(t); setError(''); }} editable={!loading} style={styles.input}/>

        <Text style={styles.label}>Status</Text>
        <View style={styles.typeRow}>
          {DISCOUNT_STATUSES.map((s) => (<Pressable key={s} onPress={() => setStatus(s)} disabled={loading} style={[
                styles.typeBtn,
        ...
        <Pressable onPress={openItemSearch} style={({ pressed }) => [styles.itemDropdownTrigger, pressed && styles.itemDropdownTriggerPressed]}>
        ...
        {selectedItems.length > 0 ? (<View style={styles.selectedChips}>
            {selectedItems.map((s) => (<View key={s.itemId} style={styles.chip}>
                <Text style={styles.chipText} numberOfLines={1}>{s.itemName}</Text>
                <Pressable onPress={() => removeSelectedItem(s.itemId)} hitSlop={8} style={styles.chipRemove}>
                  <MaterialIcons name="close" size={18} color={colors.textPrimary}/>
                </Pressable>
              </View>))}
          </View>) : null}

        <Modal visible={itemSearchOpen} transparent animationType="fade" onRequestClose={() => setItemSearchOpen(false)}>
          <Pressable style={styles.modalOverlay} onPress={() => setItemSearchOpen(false)}>
            <Pressable style={styles.modalContent} onPress={() => { }}>
              ...
              <TextInput style={styles.searchInput} placeholder="Type item name…" placeholderTextColor={colors.textSecondary} value={itemSearchQuery} onChangeText={setItemSearchQuery} autoCapitalize="none" autoCorrect={false}/>
              {itemSearchLoading ? (<View style={styles.searchLoading}>
                  <ActivityIndicator size="small" color={colors.primary}/>
                  <Text style={styles.searchLoadingText}>Searching…</Text>
                </View>) : (<View style={styles.searchResults}>
                  {itemSearchResults.length === 0 ? (<Text style={styles.searchEmpty}>
                      {itemSearchQuery.trim() ? 'No items match. Try another name.' : 'Type to search items.'}
                    </Text>) : (itemSearchResults.map((item) => (<Pressable key={item.itemId} onPress={() => {
                    ...
                    setItemSearchOpen(false);
                        ...
                        {item.price != null ? (<Text style={styles.searchResultPrice}>LKR {Number(item.price).toLocaleString()}</Text>) : null}
                      </Pressable>)))}
                </View>)}
              <Pressable onPress={() => setItemSearchOpen(false)} style={styles.modalCloseBtn}>
        ...
        <PrimaryButton title="Save changes" onPress={handleSave} loading={loading} disabled={loading || deleting} style={styles.submitBtn}/>
        <Pressable onPress={handleDelete} disabled={loading || deleting} style={[styles.deleteBtn, (loading || deleting) && styles.deleteBtnDisabled]}>
