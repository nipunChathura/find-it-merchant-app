...
function ProfileTabBarIcon({ color }: {
    ...
    if (fileName && token) {
        return (<View style={{ width: 24, height: 24, borderRadius: 12, overflow: 'hidden' }}>
        <AuthImage type="profile" fileName={fileName} token={token} style={{ width: 24, height: 24 }} resizeMode="cover" placeholder={<MaterialIcons name="person" size={22} color={color}/>}/>
      </View>);
    }
...
export default function TabLayout() {
    ...
    <Tabs screenOptions={{
      ...
      <Tabs.Screen name="sub-merchants" options={{
            title: 'Sub Merchants',
            tabBarIcon: ({ color }) => (<MaterialIcons name="people" size={24} color={isSubMerchant ? Colors.light.icon : color}/>),
        }} listeners={{
            tabPress: (e) => {
                if (isSubMerchant) {
                    e.preventDefault();
